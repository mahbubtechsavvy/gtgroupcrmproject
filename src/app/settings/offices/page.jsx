...
export default function OfficesPage() {
  ...
  // Group offices by country logic
  ...
  const getFlag = (countryName) => {
    const found = COMMON_COUNTRIES.find(c => c.name.toLowerCase() === countryName.toLowerCase());
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
          ...
          {sortedCountries.map(country => (
            <div key={country} className="office-country-group">
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px', borderBottom: '1px solid var(--color-border)', paddingBottom: '8px' }}>
                <span style={{ fontSize: '1.8rem' }}>{getFlag(country)}</span>
