...
export default function AddMemberModal({ isOpen, onClose, groupId, onMemberAdded }) {
  ...
  useEffect(() => {
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        setUsers((data || []).filter((u) => !existingMembers.includes(u.id)));
        setError('');
      } catch (err) {
  ...
  const handleAddMember = async (user) => {
    ...
    try {
      ...
      setExistingMembers([...existingMembers, user.id]);
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="w-full max-w-2xl max-h-96 rounded-3xl border border-border bg-surface-1 shadow-2xl flex flex-col overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b border-border bg-surface-2">
          <h3 className="text-lg font-bold text-white">Add Members to Group</h3>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {!loading && users.length === 0 && query && (
            <div className="p-8 text-center">
              <p className="text-sm text-text-dim">No users found or already members</p>
            </div>
          )}
          ...
          {users.map((user) => (
            <div
              key={user.id}
              className="px-6 py-3 border-b border-border hover:bg-surface-2 transition-colors flex items-center gap-3"
            >
              ...
              <button
                ...
                className="px-3 py-1 rounded-lg bg-gold text-black text-xs font-semibold hover:bg-gold/90 transition-colors disabled:opacity-50"
              >
